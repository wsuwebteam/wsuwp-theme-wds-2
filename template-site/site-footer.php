...
$address_array = array();
$address_array[] = ( $name ) ? '<span class="wsu-meta-address__title">' . $name . '</span>' : '';
$address_array[] = ( $address1 ) ? '<span class="wsu-meta-address__mail">' . $address1 . '</span>' : '';
$address_array[] = ( $address2 ) ? '<span class="wsu-meta-address__mail">' . $address2 . '</span>' : '';
$address_array[] = ( $city ) ? '<span class="wsu-meta-address__mail">' . $city . '</span>' : '';
$address_array[] = ( $state && $zip ) ? '<span class="wsu-meta-address__mail">' . $state . ' ' . $zip . '</span>' : '';
$address_array[] = ( $phone ) ? '<span class="wsu-meta-address__phone"><a href="tel:' . $phone . '">' . $phone . '</a></span>' : '';
$address_array[] = ( $email ) ? '<span class="wsu-meta-address__email"><a href="mailto:' . $email . '">' . $email . '</a></span>' : '';

array_filter( $address_array );
...
<footer class="wsu-footer-site<?php if ( ! empty( get_theme_mod( 'wsu_wds_site_footer_style', false ) ) ) : ?> wsu-footer-site--<?php echo esc_attr( get_theme_mod( 'wsu_wds_site_footer_style', '' ) ); ?><?php endif; ?>">
	<div class="wsu-footer-site__content">
		...
		<div class="wsu-meta-address">
			<?php echo wp_kses_post( implode( ', ', $address_array ) ); ?>
		</div>
		<?php get_template_part( 'template-component/component-site-social' ); ?>
		<svg role="img" aria-labelledby="wsuLogoTitle wsuLogoDesc" class="wsu-coug-head wsu-coug-head--white" enable-background="new 0 0 70.2 69.6" version="1.1" viewBox="0 0 70.2 69.6" xml:space="preserve" xmlns="http://www.w3.org/2000/svg">
